...
def blocktridiagsolve(b, a, c, d, method='cyclic'):
    """
    ...
    where B_i, A_i and C_i and m-by-m matrices. See `tridiagsolve` for the case of 1-by-1 blocks.

    :param b: main diagonal, (B_1, ..., B_n), of size (B x M x M x N)
    :param a: lower diagonal, (A_2, ..., A_n), of size (B x M x M x N-1)
    :param c: upper diagonal, (C_1, ..., C_{n-1}), of size (B x M x M x N-1)
    :param d: right-hand-size, (d_1, ..., d_mn), of size (B x MN x K)
    :param method: 'cyclic' or 'thomas' or 'linalg'
    :return: x = (x_1, ..., x_n) of size (B x MN x K)
    """

    assert len(d.shape) == 3, "argument 'd' must have shape (B, MN, K) or (B, MN, 1)"
    B, MN, K = d.shape
    assert len(b.shape) == 4, "argument 'b' must have shape (B, M, M, N)"
    M, N = b.shape[1], b.shape[3]
    assert M * N == MN, "argument 'd' must have shape (B, MN, K) or (B, MN, 1)"
    assert b.shape == (B, M, M, N), "argument 'b' must have shape (B, M, M, N)"
    assert a.shape == (B, M, M, N-1), "argument 'a' must have shape (B, M, M, N-1)"
    assert c.shape == (B, M, M, N-1), "argument 'c' must have shape (B, M, M, N-1)"

    if method == 'cyclic':
        # TODO
        raise NotImplementedError("'{}' not implemented yet".format(method))
...
class BlockTriDiagSolveFcn(torch.autograd.Function):
    ...
    def backward(ctx, grad_x):
        ...
        B, M, _, N = b.shape
        w = blocktridiagsolve(b.transpose(1, 2), c.transpose(1, 2), a.transpose(1, 2), grad_x, ctx.method)
...
if __name__ == '__main__':
    ...
    b = 0.1 * torch.rand((B, M, M, N), dtype=type, device=device) + 2.0 * torch.eye(M, dtype=type, device=device).view(1, M, M, 1)
    a = -1.0 * torch.rand((B, M, M, N - 1), dtype=type, device=device)
    c = -1.0 * torch.rand((B, M, M, N - 1), dtype=type, device=device)
    ...
    b = 0.1 * torch.rand((B, M, M, N), dtype=type, device=device, requires_grad=True) + 2.0 * torch.eye(M, dtype=type, device=device).view(1, M, M, 1)
    a = -1.0 * torch.rand((B, M, M, N - 1), dtype=type, device=device, requires_grad=True)
    c = -1.0 * torch.rand((B, M, M, N - 1), dtype=type, device=device, requires_grad=True)
